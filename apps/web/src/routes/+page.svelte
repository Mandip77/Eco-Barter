<script lang="ts">
  ...
  async function loadListings() {
    ...
    try {
      ...
      if (resp.ok) {
        ...
        listings = data.map((dbItem: any) => {
          ...
          return {
            ...
            title: dbItem.title,
            user:
              dbItem.owner_id === authState.user?.id
            ...
            userInitials:
              dbItem.owner_id === authState.user?.id
                ? authState.user?.username?.substring(0, 2).toUpperCase() ||
                  "ME"
                : dbItem.owner_id.substring(0, 2).toUpperCase(),
            cat: dbItem.category,
            emoji: dbItem.emoji || "📦",
      ...
      listingsError =
        "Unable to reach the catalog service. Check your connection or try again shortly.";
    } finally {
  ...
  function handleImageSelect(e: Event) {
    ...
    if (file.size > 1024 * 1024) {
      showToast("Image must be under 1 MB.", "error");
      return;
    }
    newListingImage = file;
    const reader = new FileReader();
    reader.onload = (ev) => {
      newListingImagePreview = (ev.target?.result as string) || "";
    };
  ...
  async function getAISuggestion(listingId: string) {
    ...
    try {
      const resp = await fetch(
        `/api/v1/catalog/products/${listingId}/suggest`,
        {
          method: "POST",
          headers: { Authorization: `Bearer ${authState.token}` },
        },
  ...
  async function saveEditListing() {
    ...
    try {
      ...
      const resp = await fetch(
        `/api/v1/catalog/products/${editingListing.id}`,
        {
          method: "PUT",
          headers: {
      ...
      if (resp.ok) {
        if (editImage) {
          ...
          const imgResp = await fetch(
            `/api/v1/catalog/products/${editingListing.id}/image`,
            {
              method: "POST",
              headers: { Authorization: `Bearer ${authState.token}` },
              body: fd,
          ...
          if (imgResp.ok) {
            ...
            editImagePreview = d.image_data || editImagePreview;
          }
        }
  ...
  function shareListing(l: any) {
    ...
    navigator.clipboard
      .writeText(url)
      .then(() => showToast("Listing link copied!", "success"));
  }
  ...
  async function bumpListing(e: MouseEvent, l: any) {
    ...
    try {
      ...
      if (resp.ok) {
        showToast(`"${l.title}" bumped — active for 30 more days!`, "success");
        await loadListings();
      } else {
        showToast("Bump failed.", "error");
      }
    } catch {
      showToast("Network error.", "error");
    }
  ...
  async function submitCounter(tradeId: number) {
    if (!counterItem) {
      showToast("Select an item to counter with.", "error");
      return;
    }
    try {
      ...
      if (resp.ok) {
        showCounterPanel = null;
        counterItem = "";
        showToast("Counter-proposal sent!", "success");
      } else {
        showToast("Failed to send counter.", "error");
      }
    } catch {
      showToast("Network error.", "error");
    }
  ...
  async function loadReviews() {
    ...
    try {
      const resp = await fetch(
        `/api/v1/reputation/reviews/${authState.user.username}`,
      );
      if (resp.ok) {
        const data = await resp.json();
  ...
  function shareProfile() {
    navigator.clipboard
      .writeText(`${window.location.origin}?user=${authState.user?.username}`)
  ...
  async function changePassword() {
    ...
    if (!pwForm.current || !pwForm.next) {
      pwError = "Please fill in all fields.";
      return;
    }
  ...
  async function deleteAccount() {
    ...
    try {
      ...
      if (resp.status === 204) {
        ...
        deleteError = d.detail || "Failed to delete account.";
      }
    } catch {
  ...
  function selectOffer(id: string) {
    selectedOfferItem = id;
  }
  function proposeTrade() {
    if (!selectedOfferItem) {
      showToast("Please select a listing to offer");
      return;
    }
    showToast(
      `Trade preference registered for ${selectedListing.title}! The EcoBarter Matrix is calculating loops…`,
    );
  ...
  async function addListing() {
    ...
    if (!newListingImage) {
      showToast("Please add a photo of your item before posting.", "error");
      return;
    }
    try {
      const payload = {
      ...
      if (resp.ok) {
        ...
        if (newListingImage) {
          ...
          const imgResp = await fetch(
            `/api/v1/catalog/products/${dbItem._id}/image`,
            {
              method: "POST",
              headers: { Authorization: `Bearer ${authState.token}` },
              body: fd,
          ...
          if (imgResp.ok) {
            ...
            imageData = imgData.image_data || null;
          }
        }
        listings = [
          {
            ...
            userInitials:
              authState.user?.username?.substring(0, 2).toUpperCase() || "ME",
            cat: dbItem.category,
            emoji: dbItem.emoji,
            desc: dbItem.description,
            wants: dbItem.wants?.preferences?.query || "Open to offers",
  ...
  onMount(async () => {
    ...
    if (storedLoc) {
      try {
        userLocation = JSON.parse(storedLoc);
      } catch {
        /* ignore */
      }
    }
    ...
    if (authState.user) {
      await Promise.all([
        ...
        fetch(`/api/v1/reputation/${authState.user.username}`)
          ...
          .then((d) => {
            if (d) {
              userReputation = d.eigentrust_score ?? 10.0;
              userRank = d.rank ?? "Novice";
            }
          })
          .catch(() => {}),
      ]);
    ...
    centrifuge = new Centrifuge(
      `${wsProto}//${window.location.host}/connection/websocket`,
    );
    centrifuge.connect();
    ...
    if (authState.user) {
      try {
        ...
        if (propResp.ok) {
          ...
          tradeHistory = proposals
            ...
            .map((p: any) => ({
              id: p.id,
              with: [p.user_a, p.user_b, p.user_c, p.user_d]
                .filter(Boolean)
                .join(", "),
          ...
          chats = active.map((p: any) => ({
            ...
            proposal: {
              me: [p.item_a],
              them: [p.item_b, p.item_c, p.user_d ? p.item_d : null].filter(
                Boolean,
              ),
            },
              ...
              sub.on("publication", (ctx) => {
                ...
                if (ind > -1)
                  chats[ind].messages = [
                    ...
                    {
                      from:
                        ctx.data.from === authState.user?.username
                          ? "me"
                          : ctx.data.from,
                      text: ctx.data.text,
                      time: new Date().toLocaleTimeString([], {
      ...
      hubSub.on("publication", (ctx) => {
        ...
        if (!p) return;
        const isParticipant = [p.user_a, p.user_b, p.user_c, p.user_d].includes(
          authState.user?.username,
        );
        if (isParticipant) {
          if (
            p.status === "pending" &&
            !chats.find((c: any) => c.id === p.id)
          ) {
            chats = [
              {
                id: p.id,
            ...
            if (currentPage !== "chat") {
              pendingMatchCount++;
              showToast("New trade loop matched!", "match");
            }
          } else if (p.status === "completed") {
            showToast(
              `Trade Loop #${p.ID} completed! +5 kg CO₂ saved.`,
              "success",
            );
          }
        }
...
<nav>
  ...
  <div class="nav-links">
    <button
      class="nav-btn {currentPage === 'browse' ? 'active' : ''}"
      onclick={() => setPage("browse")}>Browse</button
    >
    <button
      class="nav-btn {currentPage === 'chat' ? 'active' : ''}"
      onclick={() => setPage("chat")}
    >
      Negotiation {#if pendingMatchCount > 0}<span class="badge"
          >{pendingMatchCount}</span
        >{/if}
    </button>
    <button
      class="nav-btn {currentPage === 'profile' ? 'active' : ''}"
  ...
  <div class="nav-right">
    ...
    {#if authState.user}
      <button
        ...
        onclick={() => (modals.newListing = true)}>+ New Listing</button
      >
      <div class="avatar-wrap">
        ...
        >
          {#if profileEmoji}<span style="font-size:1.05rem;line-height:1"
              >{profileEmoji}</span
            >{:else}{authState.user.username.substring(0, 2).toUpperCase()}{/if}
        </button>
        {#if showAvatarMenu}
          <div class="avatar-menu">
            <div class="avatar-menu-header">
              <div
                ...
                style="background:linear-gradient(135deg,{profileColor},{profileColor}cc)"
              >
                {#if profileEmoji}<span style="font-size:1.2rem"
                    >{profileEmoji}</span
                  >{:else}{authState.user.username
                    .substring(0, 2)
                    .toUpperCase()}{/if}
              </div>
            ...
            <button
              ...
              onclick={() => {
                ...
                profileTab = "overview";
                showAvatarMenu = false;
              }}><span class="menu-icon">👤</span> View Profile</button
            >
            <button
              class="avatar-menu-item"
              onclick={() => {
                ...
                profileTab = "edit";
                showAvatarMenu = false;
              }}><span class="menu-icon">✏️</span> Edit Profile</button
            >
            <button
              class="avatar-menu-item"
              onclick={() => {
                ...
                profileTab = "security";
                showAvatarMenu = false;
              }}><span class="menu-icon">🔒</span> Change Password</button
            >
            <button
              class="avatar-menu-item"
              onclick={() => {
                ...
                profileTab = "settings";
                showAvatarMenu = false;
              }}><span class="menu-icon">⚙️</span> Settings</button
            >
            <div class="avatar-menu-sep"></div>
            <button class="avatar-menu-item danger" onclick={handleLogout}
              ><span class="menu-icon">🚪</span> Sign Out</button
            >
          </div>
        {/if}
      </div>
...
<nav class="bottom-nav" aria-label="Mobile navigation">
  <button
    class="bottom-nav-btn {currentPage === 'browse' ? 'active' : ''}"
    onclick={() => setPage("browse")}
  >
    <span class="bn-icon">🏪</span>Browse
  </button>
  <button
    class="bottom-nav-btn {currentPage === 'chat' ? 'active' : ''}"
    onclick={() => setPage("chat")}
  >
    <span class="bn-icon">💬</span>Negotiate
    {#if pendingMatchCount > 0}<span class="badge">{pendingMatchCount}</span
      >{/if}
  </button>
  <button
    ...
    onclick={() =>
      authState.user
        ? (setPage("browse"), (modals.newListing = true))
        : goto("/login")}
  >
    <span
      class="bn-icon"
      style="background:var(--accent);color:#fff;border-radius:50%;width:34px;height:34px;display:flex;align-items:center;justify-content:center;font-size:1.1rem"
      >+</span
    >
    Post
  </button>
  <button
    class="bottom-nav-btn {currentPage === 'profile' ? 'active' : ''}"
...
<div class="page {currentPage === 'browse' ? 'active' : ''}" id="page-browse">
  <div class="hero">
    ...
    <div class="hero-btns">
      <button
        ...
        onclick={() =>
          authState.user ? (modals.newListing = true) : goto("/login")}
      ...
      <button
        ...
        onclick={() =>
          document
            ...
            ?.scrollIntoView({ behavior: "smooth" })}>Browse Swaps</button
      >
    </div>
  </div>
  ...
  <div id="listings-section">
    <div class="section-header">
      ...
      {#if !isLoadingListings && !listingsError}
        <span style="font-size:0.82rem;color:var(--text3)"
          >{filteredListings.length} listings{userLocation
    ...
    <div class="filters">
      ...
      {#if userLocation}
        <div class="location-bar active">
          <span>📍 Near you</span>
          <select
            class="location-radius-select"
            bind:value={locationRadius}
            onchange={() => loadListings()}
          >
            {#each [1, 5, 10, 25, 50] as r}<option value={r}>{r} km</option
              >{/each}
          </select>
          <button
            class="location-clear-btn"
            onclick={clearLocation}
            title="Clear location">×</button
          >
        </div>
      {:else}
        <button
      ...
      <div class="card-grid">
        {#each Array(6) as _}
          <div class="skeleton-card">
            ...
            <div
              style="padding:20px 22px 22px;display:flex;flex-direction:column;gap:10px;"
            >
    ...
    {:else if listingsError}
      <div
        style="background:rgba(239,68,68,0.06);border:1.5px solid rgba(239,68,68,0.18);border-radius:var(--radius-sm);padding:16px 20px;display:flex;align-items:center;gap:12px;font-size:0.88rem;color:#dc2626;"
      >
        <span style="flex:1">{listingsError}</span>
        <button
          onclick={loadListings}
          style="font-weight:700;text-decoration:underline;background:none;border:none;cursor:pointer;color:inherit;"
          >Retry</button
        >
      </div>
    {:else}
      <div class="card-grid">
        {#each filteredListings as l}
          <button class="listing-card" onclick={() => openListing(l.id)}>
            <div
              class="listing-img"
              style={l.image_data ? "padding:0;overflow:hidden" : ""}
            >
              {#if l.image_data}
                <img
                  src={l.image_data}
              ...
              <div
                ...
                onkeydown={(e) =>
                  e.key === "Enter" &&
                  toggleSave(e as unknown as MouseEvent, l.id)}
                title={savedIds.has(l.id)
                  ? "Remove from saved"
                  : "Save listing"}
              >
                {savedIds.has(l.id) ? "🔖" : "🏷️"}
              ...
              {#if l.distKm !== null}
                <span class="distance-badge"
                  >{l.distKm < 1
                    ? `${(l.distKm * 1000).toFixed(0)} m`
                    : `${l.distKm.toFixed(1)} km`}</span
                >
              {/if}
            </div>
            <div class="listing-body">
              ...
              >
                <div class="listing-title" style="flex:1;margin:0">
                  {l.title}
                </div>
                <span
                  class="condition-badge"
                  style="background:{conditionColors[
                    l.condition
                  ]}18;color:{conditionColors[
                    l.condition
                  ]};border-color:{conditionColors[l.condition]}44"
                  >{l.condition}</span
                >
              </div>
              <div class="listing-user">
                by {l.user}{#if l.mine}
                  <span
                    ...
                    >You</span
                  >{/if}{#if l.view_count > 0}<span
                    style="margin-left:6px;font-size:0.74rem;color:var(--text3)"
                    >👁 {l.view_count}</span
                  >{/if}
              </div>
              <div class="listing-tags">
                {#each l.tags as t}<span
                    class="tag {t === 'Eco-friendly' ? 'green' : ''}">{t}</span
                  >{/each}
              </div>
              <div class="listing-wants">
                Wants: <strong>{l.wants.split(",")[0]}…</strong>
              </div>
            </div>
          </button>
        {/each}
        {#if filteredListings.length === 0}
          <div
            style="grid-column:1/-1;text-align:center;padding:60px 20px;color:var(--text3);font-size:0.95rem;"
          >
            No listings match{userLocation ? " near your location" : ""}.
            <button
              onclick={() =>
                authState.user ? (modals.newListing = true) : goto("/login")}
              style="color:var(--accent);font-weight:600;background:none;border:none;cursor:pointer;text-decoration:underline;"
              >Be the first to list</button
            >.
          </div>
        {/if}
      </div>
...
<div class="page {currentPage === 'chat' ? 'active' : ''}" id="page-chat">
  <div class="chat-layout {activeChatId ? 'has-active-chat' : ''}">
    <div class="chat-sidebar">
        ...
        >
          ...
          <div class="chat-item-info">
            ...
            <div class="chat-item-preview">
              {#if c.scheduledAt}📅 {new Date(c.scheduledAt).toLocaleDateString(
                  [],
                  {
                    month: "short",
                    day: "numeric",
      ...
      {#if chats.length === 0}
        <div
          style="padding:24px;font-size:0.84rem;color:var(--text3);text-align:center;line-height:1.55;"
        >
          No active trade loops yet.<br />Browse listings and propose a trade to
          start.
        </div>
      {/if}
    </div>
    ...
    <div class="chat-main" id="chat-main">
        ...
        <div class="chat-header">
          ...
          >
            <button
              ...
              onclick={() =>
                (showScheduler =
                  showScheduler === activeChatId ? null : activeChatId)}
            >
        ...
        {#if showCounterPanel === activeChatId}
          <div class="counter-panel">
            <div style="font-weight:700;font-size:0.88rem;margin-bottom:10px">
              🔄 Counter-Offer — pick your item
            </div>
            <div style="display:flex;flex-wrap:wrap;gap:7px;margin-bottom:10px">
              {#each myItems as i}
                <button
                  class="proposal-item"
                  style={counterItem === i.title
                    ? "border-color:var(--accent);background:var(--accent-light);color:var(--accent2)"
                    : ""}
                  onclick={() => (counterItem = i.title)}
                >
            ...
            <div style="display:flex;gap:8px">
              <button
                class="btn btn-primary btn-sm"
                onclick={() => submitCounter(activeChatId as number)}
                disabled={!counterItem}>Send Counter</button
              >
              <button
                class="btn btn-outline btn-sm"
                onclick={() => {
        ...
        {#if showScheduler === activeChatId}
          <div class="schedule-panel">
            ...
            {#if activeChat.scheduledAt}
              <div
                style="font-size:0.83rem;color:var(--accent);margin-bottom:8px"
              >
                Currently: {new Date(activeChat.scheduledAt).toLocaleString()}
              </div>
            {/if}
            <div style="display:flex;gap:8px;align-items:center">
              <input
                ...
                style="flex:1;font-size:0.85rem;padding:7px 10px;border:1.5px solid var(--border);border-radius:var(--radius-sm);background:var(--surface2);color:var(--text);outline:none;"
              />
              <button
                class="btn btn-primary btn-sm"
                onclick={() => scheduleTrade(activeChatId as number)}
              ...
              <button
                class="btn btn-outline btn-sm"
                onclick={() => (showScheduler = null)}>Cancel</button
              >
            </div>
          </div>
        {/if}
        ...
        <div class="chat-messages">
          {#if activeChat.proposal}
            <div class="proposal-bar">
              <span
                style="color:var(--accent);font-weight:700;font-size:0.84rem;flex-shrink:0"
                >📦 Proposal</span
              >
              <div class="proposal-items">
                {#each activeChat.proposal.me as i}<div class="proposal-item">
                    <span>{i.split(" ")[0]}</span>{i
                      .split(" ")
                      .slice(1)
                      .join(" ")}
                  </div>{/each}
              ...
              <div class="proposal-items">
                {#each activeChat.proposal.them as i}<div class="proposal-item">
                    <span>{i.split(" ")[0]}</span>{i
                      .split(" ")
                      .slice(1)
                      .join(" ")}
              ...
              <button
                class="btn btn-primary btn-sm"
                onclick={() => acceptTrade(activeChatId as number)}
                >Accept</button
              >
              <button
                class="btn btn-outline btn-sm"
                onclick={() => {
                  showCounterPanel =
                    showCounterPanel === activeChatId ? null : activeChatId;
                  counterItem = "";
                }}>🔄 Counter</button
              >
            </div>
          ...
          {#each activeChat.messages as m}
            <div
              class="msg {m.from === 'me' ? 'mine' : ''} {m.from === 'ai'
                ? 'ai'
                : ''}"
            >
              <div
                ...
                style={m.from === "me"
                  ...
                  : m.from === "ai"
                    ? "background:#fefce8;color:#b45309;border-color:#fde68a"
                    : ""}
              >
                {m.from === "me"
                  ? "Me"
      ...
      {:else}
        <div
          style="flex:1;display:flex;align-items:center;justify-content:center;color:var(--text3);font-size:0.9rem"
        >
          Select a negotiation to start
        </div>
      {/if}
    </div>
  </div>
...
<div class="page {currentPage === 'profile' ? 'active' : ''}" id="page-profile">
  {#if !authState.user}
    <div style="text-align:center;padding:80px 20px">
      ...
      <h2
        style="font-family:'Outfit',sans-serif;font-size:1.6rem;margin-bottom:8px"
      >
  ...
  {:else}
    <div class="profile-card-new">
      ...
      <div class="profile-card-body-new">
        <div class="profile-avatar-wrap">
          <button
            ...
            title="Change avatar"
          >
            {#if profileEmoji}<span style="font-size:2.6rem;line-height:1"
                >{profileEmoji}</span
              >{:else}{authState.user.username
                .substring(0, 2)
                .toUpperCase()}{/if}
          ...
          {#if showEmojiPicker}
            ...
            >
              ...
              <div class="emoji-grid">
                {#each avatarEmojis as e}<button
                    class="emoji-opt {profileEmoji === e ? 'selected' : ''}"
                    onclick={() => selectEmoji(e)}>{e}</button
                  >{/each}
              </div>
              <div class="picker-section-title" style="margin-top:12px">
                Color
              </div>
              <div class="color-grid">
                {#each avatarColors as c}<button
                    class="color-opt {profileColor === c ? 'selected' : ''}"
                    style="background:{c}"
                    onclick={() => selectColor(c)}
                  ></button>{/each}
              </div>
              {#if profileEmoji}<button
                  class="btn btn-outline btn-sm"
                  style="margin-top:10px;width:100%;font-size:0.78rem"
                  onclick={() => {
                    profileEmoji = "";
                    localStorage.removeItem("profile_emoji");
                    showEmojiPicker = false;
        ...
        <div style="flex:1;min-width:0">
          ...
          >
            <h2
              style="font-size:1.65rem;font-weight:900;letter-spacing:-0.6px;font-family:'Outfit',sans-serif"
            >
              {authState.user.username}
            </h2>
            <span class="rank-pill">{userRank}</span>
          </div>
          <p style="font-size:0.85rem;color:var(--text3);margin-bottom:8px">
            {authState.user.email}
          </p>
          {#if profileBio}<p
              style="font-size:0.93rem;color:var(--text2);line-height:1.6;max-width:500px;margin-bottom:12px"
            >
              {profileBio}
            </p>
          {:else}<p
              style="font-size:0.85rem;color:var(--text3);font-style:italic;margin-bottom:12px"
            >
              No bio yet — <button
                onclick={() => (profileTab = "edit")}
                style="color:var(--accent);background:none;border:none;cursor:pointer;text-decoration:underline;font-size:inherit;font-style:normal"
                >add one</button
              >
            </p>{/if}
          <div style="display:flex;gap:8px;flex-wrap:wrap">
            ...
            <button
              class="btn btn-outline btn-sm"
              onclick={() => (profileTab = "edit")}>Edit Profile</button
            >
          </div>
        </div>
      </div>
      <div class="profile-stats-row">
        ...
        <div class="profile-stat-div"></div>
        <div class="profile-stat-item">
          <div class="profile-stat-val" style="color:#10b981">
            {savedIds.size}
          </div>
    ...
    <div class="profile-subtabs">
      {#each [["overview", "Overview"], ["listings", "My Listings"], ["saved", "Wishlist"], ["history", "History"], ["edit", "Edit Profile"], ["security", "Security"], ["settings", "Settings"]] as [tab, label]}
        <button
          class="profile-subtab {profileTab === tab ? 'active' : ''}"
          onclick={() => (profileTab = tab as typeof profileTab)}
          >{label}</button
        >
      {/each}
    </div>
    ...
    {#if profileTab === "overview"}
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px">
          ...
          <div style="display:flex;flex-direction:column;gap:16px">
            <div>
              <div class="eco-bar-row" style="margin-bottom:7px">
                <span
                  style="font-size:0.83rem;color:var(--text2);font-weight:600"
                  >CO₂ Saved</span
                ><span style="font-size:0.83rem;font-weight:700;color:#10b981"
                  >{chats.length * 5} kg</span
                >
              </div>
              <div class="eco-progress-track">
                <div
                  class="eco-progress-fill"
                  style="width:{Math.min(chats.length * 12, 100)}%"
                ></div>
              </div>
            </div>
            <div class="eco-bar-row">
              <span style="font-size:0.83rem;color:var(--text2);font-weight:600"
                >Items Diverted from Landfill</span
              ><span
                style="font-size:0.83rem;font-weight:700;color:var(--accent)"
                >{myItems.length + chats.length}</span
              >
            </div>
            <div class="eco-bar-row">
              <span style="font-size:0.83rem;color:var(--text2);font-weight:600"
              ...
              ><span
                class="rank-pill"
                style="font-size:0.73rem;padding:2px 10px">{userRank}</span
              >
            </div>
            <div class="eco-bar-row">
              <span style="font-size:0.83rem;color:var(--text2);font-weight:600"
                >EigenTrust Score</span
              ><span style="font-size:0.83rem;font-weight:700"
                >{userReputation.toFixed(1)} / 100</span
              >
            </div>
          </div>
        </div>
        <div>
          <div class="section-header">
            <div class="section-title">Reviews ({userReviews.length})</div>
          </div>
          ...
          {:else}
            <div class="review-card">
              ...
              >
                No reviews yet. Complete trades to earn reviews.
              </p>
            </div>
          {/if}
        </div>
    ...
    {:else if profileTab === "listings"}
      <div class="card" style="padding:0;overflow:hidden">
        <div
          style="padding:18px 24px;border-bottom:1px solid var(--border);display:flex;align-items:center;justify-content:space-between"
        >
        ...
        {#each myItems as l}
          <div
            style="display:flex;align-items:center;gap:14px;padding:14px 24px;border-bottom:1px solid var(--border)"
          >
            <div
              style="width:46px;height:46px;background:var(--accent-light);border-radius:var(--radius-sm);display:flex;align-items:center;justify-content:center;flex-shrink:0;overflow:hidden"
            >
              {#if l.image_data}<img
                  src={l.image_data}
                  alt={l.title}
                  style="width:100%;height:100%;object-fit:cover"
                />{:else}<span style="font-size:1.9rem">{l.emoji}</span>{/if}
            </div>
            <div style="flex:1;min-width:0">
              ...
              >
                ...
                {#if l.expires_at}
                  <span
                    class="expiry-badge {new Date(l.expires_at) <
            ...
            {#if l.expires_at}<button
                class="btn btn-outline btn-sm"
                onclick={(e) => bumpListing(e, l)}>Bump</button
              >{/if}
            <button
              class="btn btn-outline btn-sm"
              onclick={() => openEditModal(l)}>Edit</button
            >
            <button
              class="btn btn-outline btn-sm"
              style="color:var(--warn);border-color:rgba(225,29,72,0.3)"
              onclick={(e) => deleteListing(e, l)}>Delete</button
            >
          </div>
        {/each}
        {#if myItems.length === 0}<div
            style="text-align:center;padding:52px 24px;color:var(--text3)"
          >
            <div style="font-size:2.8rem;margin-bottom:12px">📦</div>
            <p style="font-size:0.9rem;margin-bottom:16px">No listings yet.</p>
            <button
              class="btn btn-primary btn-sm"
              onclick={() => (modals.newListing = true)}
    ...
    {:else if profileTab === "saved"}
      <div class="card" style="padding:0;overflow:hidden">
          ...
          {#if batchSelected.size > 0}
            <div style="display:flex;gap:8px;align-items:center">
              ...
              <button
                ...
                disabled={batchProposing}>Propose Selected</button
              >
              <button
                class="btn btn-outline btn-sm"
                onclick={() => (batchSelected = new Set())}>Clear</button
              >
            </div>
          {:else}
            <span style="font-size:0.78rem;color:var(--text3)"
              >Tap ☐ to multi-select</span
            >
          {/if}
        </div>
        {#each listings.filter((l) => savedIds.has(l.id)) as l}
          <!-- svelte-ignore a11y_interactive_supports_focus -->
          <div
            class="listing-row {batchSelected.has(l.id) ? 'selected' : ''}"
            role="button"
            tabindex="0"
            onclick={() => openListing(l.id)}
            onkeydown={(e) => e.key === "Enter" && openListing(l.id)}
          >
            <button
              class="batch-check"
              onclick={(e) => {
                e.stopPropagation();
                toggleBatchSelect(l.id);
              }}
              style="background:none;border:1.5px solid {batchSelected.has(l.id)
                ? 'var(--accent)'
                : 'var(--border)'};border-radius:6px;width:22px;height:22px;flex-shrink:0;display:flex;align-items:center;justify-content:center;font-size:0.85rem;cursor:pointer;color:{batchSelected.has(
                l.id,
              )
                ? 'var(--accent)'
                : 'var(--text3)'}"
            ...
            <div
              style="width:40px;height:40px;background:var(--accent-light);border-radius:var(--radius-sm);display:flex;align-items:center;justify-content:center;flex-shrink:0;overflow:hidden"
            >
              {#if l.image_data}<img
                  src={l.image_data}
                  alt={l.title}
                  style="width:100%;height:100%;object-fit:cover"
                />{:else}<span style="font-size:1.5rem">{l.emoji}</span>{/if}
            </div>
            <div style="flex:1;min-width:0">
              <div style="font-weight:700;font-size:0.93rem">{l.title}</div>
              <div class="text-sm text-muted">
                by {l.user} · {l.condition}{#if l.view_count > 0}
                  · 👁 {l.view_count}{/if}
              </div>
            </div>
            <span
            ...
            <button
              class="btn btn-outline btn-sm"
              onclick={(e) => toggleSave(e, l.id)}>Unsave</button
            >
          </div>
        {/each}
        {#if savedIds.size === 0}<div
            style="text-align:center;padding:52px 24px;color:var(--text3)"
          >
            <div style="font-size:2.8rem;margin-bottom:12px">🏷️</div>
            <p>No saved listings yet. Tap 🏷️ on any listing to save it.</p>
          </div>{/if}
      </div>
    {:else if profileTab === "history"}
      <div class="card" style="padding:0;overflow:hidden">
        ...
        >
          ...
          {#if tradeHistory.length > 0}<span
              style="font-size:0.82rem;color:var(--text3)"
              >{tradeHistory.filter((t) => t.status === "completed").length} completed
              · {tradeHistory.filter((t) => t.status === "cancelled").length} cancelled</span
            >{/if}
        </div>
        {#each tradeHistory as t}
          <div class="trade-hist-item">
            ...
            <div style="flex:1;min-width:0">
              ...
              <div class="text-sm text-muted">
                with {t.with}{t.created_at
                  ? ` · ${new Date(t.created_at).toLocaleDateString([], { month: "short", day: "numeric", year: "numeric" })}`
                  : ""}
              </div>
            </div>
            <span
              class="tag {t.status === 'completed' ? 'green' : ''}"
              style="flex-shrink:0;text-transform:capitalize">{t.status}</span
            >
            {#if t.status === "completed"}<span
                style="font-size:0.78rem;color:#10b981;font-weight:600;white-space:nowrap"
                >+5 kg CO₂</span
              >{/if}
          </div>
        {/each}
        {#if tradeHistory.length === 0}
          <div style="text-align:center;padding:52px 24px;color:var(--text3)">
            ...
            <p style="font-size:0.9rem">
              No completed or cancelled trades yet.
            </p>
          </div>
        {/if}
      </div>
    {:else if profileTab === "edit"}
      <div class="card" style="max-width:560px;padding:28px">
        ...
        <div class="form-group">
          <label for="edit-username">Username</label><input
            id="edit-username"
            type="text"
          ...
          />
          <p style="font-size:0.76rem;color:var(--text3);margin-top:5px">
            Username cannot be changed.
          </p>
        </div>
        <div class="form-group">
          <label for="edit-email">Email</label><input
            id="edit-email"
            type="email"
        ...
        <div class="form-group">
          <label for="edit-bio">Bio</label><textarea
            id="edit-bio"
            bind:value={editBio}
        ...
        <div style="display:flex;gap:10px">
          <button class="btn btn-primary" onclick={saveEditProfile}
            >Save Changes</button
          ><button
            class="btn btn-outline"
            onclick={() => {
    ...
    {:else if profileTab === "security"}
      <div class="card" style="max-width:460px;padding:28px">
        ...
        <p style="font-size:0.84rem;color:var(--text3);margin-bottom:22px">
        ...
        <div class="form-group">
          <label for="pw-current">Current Password</label><input
            id="pw-current"
            type="password"
        ...
        <div class="form-group">
          <label for="pw-new">New Password</label><input
            id="pw-new"
            type="password"
        ...
        <div class="form-group">
          <label for="pw-confirm">Confirm New Password</label><input
            id="pw-confirm"
            type="password"
        ...
        >
        <div
          style="margin-top:36px;padding-top:24px;border-top:1px solid var(--border)"
        >
          <div
            style="font-weight:700;font-size:0.92rem;color:var(--warn);margin-bottom:6px"
          >
            Danger Zone
          </div>
          <p style="font-size:0.82rem;color:var(--text3);margin-bottom:14px">
      ...
      <div class="card" style="max-width:520px;padding:28px">
        ...
        <div class="settings-row">
          <div>
            ...
            <div style="font-size:0.8rem;color:var(--text3);margin-top:3px">
              Get notified when a K-way loop matches
            </div>
          </div>
          <button
            class="toggle {settingsNotifs ? 'on' : ''}"
            onclick={() => {
        ...
        <div class="settings-row">
          <div>
            ...
            <div style="font-size:0.8rem;color:var(--text3);margin-top:3px">
              Allow others to view your profile and reputation
            </div>
          </div>
          <button
            class="toggle {settingsPublic ? 'on' : ''}"
            onclick={() => {
        ...
        <div class="settings-row">
          <div>
            ...
            <div style="font-size:0.8rem;color:var(--text3);margin-top:3px">
              Weekly CO₂ savings and eco milestones
            </div>
          </div>
          <button
            class="toggle {settingsEcoAlerts ? 'on' : ''}"
            onclick={() => {
              settingsEcoAlerts = !settingsEcoAlerts;
              saveSettings();
            }}><span></span></button
          >
        </div>
        <div
          style="margin-top:30px;padding-top:22px;border-top:1px solid var(--border)"
        >
          <div
            class="section-title"
            style="font-size:0.9rem;margin-bottom:14px"
          >
            Account Info
          </div>
          <div style="display:flex;flex-direction:column;gap:10px">
            <div class="info-row">
              <span>User ID</span><code
                style="font-size:0.79rem;background:var(--surface2);padding:3px 9px;border-radius:6px;color:var(--text3)"
                >{authState.user.id}</code
              >
            </div>
            <div class="info-row">
              <span>Email</span><span style="color:var(--text2)"
...
<div
  class="modal-overlay {modals.newListing ? 'open' : ''}"
  onclick={closeModals}
>
  <div class="modal">
    <div class="modal-header">
      <div class="modal-title">New Listing</div>
      <button class="close-btn" onclick={() => (modals.newListing = false)}
        >×</button
      >
    </div>
    ...
    <div class="form-group">
      <label for="nl-title">Item / Service Name</label><input
        id="nl-title"
        type="text"
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">
      <div class="form-group">
        <label for="nl-cat">Category</label><select
          id="nl-cat"
          bind:value={newListing.cat}
          >{#each categories.filter((c) => c !== "All") as c}<option>{c}</option
            >{/each}</select
        >
      </div>
      <div class="form-group">
        <label for="nl-cond">Condition</label><select
          id="nl-cond"
          bind:value={newListing.condition}
          >{#each CONDITIONS as c}<option>{c}</option>{/each}</select
        >
      </div>
    </div>
    <div class="form-group">
      <label for="nl-desc">Description</label><textarea
        id="nl-desc"
        bind:value={newListing.desc}
        placeholder="Describe condition, details, etc."
      ></textarea>
    </div>
    <div class="form-group">
      <label for="nl-wants">Looking to trade for…</label><input
        id="nl-wants"
        type="text"
        bind:value={newListing.wants}
        placeholder="e.g. Camera lenses, or open to offers"
      />
    </div>
    <div class="form-group">
      <label for="nl-emoji">Emoji Icon</label><input
        id="nl-emoji"
        type="text"
        bind:value={newListing.emoji}
        placeholder="📷"
        maxlength="4"
        style="width:80px"
      />
    </div>
    <button class="btn btn-primary w-full" onclick={addListing}
      >Post Listing</button
    >
  </div>
</div>
...
<!-- svelte-ignore a11y_no_static_element_interactions -->
<div
  class="modal-overlay {modals.editListing ? 'open' : ''}"
  onclick={closeModals}
>
  <div class="modal">
    <div class="modal-header">
      <div class="modal-title">Edit Listing</div>
      <button class="close-btn" onclick={() => (modals.editListing = false)}
        >×</button
      >
    </div>
    ...
    <div class="form-group">
      ...
      <div
        class="image-upload-area"
        onclick={() => document.getElementById("el-image")?.click()}
      >
        {#if editImagePreview}
          <img
    ...
    <div class="form-group">
      <label for="el-title">Item / Service Name</label><input
        id="el-title"
        type="text"
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">
      <div class="form-group">
        <label for="el-cat">Category</label><select
          id="el-cat"
          bind:value={editForm.cat}
          >{#each categories.filter((c) => c !== "All") as c}<option>{c}</option
            >{/each}</select
        >
      </div>
      <div class="form-group">
        <label for="el-cond">Condition</label><select
          id="el-cond"
          bind:value={editForm.condition}
          >{#each CONDITIONS as c}<option>{c}</option>{/each}</select
        >
      </div>
    </div>
    <div class="form-group">
      <label for="el-desc">Description</label><textarea
        id="el-desc"
        bind:value={editForm.desc}
      ></textarea>
    </div>
    <div class="form-group">
      <label for="el-wants">Looking to trade for…</label><input
        id="el-wants"
        type="text"
        bind:value={editForm.wants}
      />
    </div>
    <div class="form-group">
      <label for="el-emoji">Emoji Icon</label><input
        id="el-emoji"
        type="text"
    ...
    <div style="display:flex;gap:10px">
      ...
      <button
        class="btn btn-outline"
        onclick={() => (modals.editListing = false)}>Cancel</button
      >
    </div>
  </div>
</div>
...
<!-- svelte-ignore a11y_no_static_element_interactions -->
<div
  class="modal-overlay {modals.listingDetail && selectedListing ? 'open' : ''}"
  onclick={closeModals}
>
  {#if selectedListing}
    <div class="modal" style="max-width:600px">
      <div class="modal-header">
        <div>
          ...
          {#if selectedListing.view_count > 0}<div
              style="font-size:0.76rem;color:var(--text3);margin-top:2px"
            >
        ...
        <div style="display:flex;gap:8px;align-items:center">
          <button
            ...
            title="Copy link">🔗 Share</button
          >
          <button
            class="close-btn"
            onclick={() => (modals.listingDetail = false)}>×</button
          >
        </div>
      </div>

      <div
        style="position:relative;font-size:4.5rem;text-align:center;padding:24px;background:linear-gradient(135deg,var(--accent-light),var(--surface2));border-radius:var(--radius-sm);margin-bottom:16px;overflow:hidden"
      >
        {#if selectedListing.image_data}
          <img
            src={selectedListing.image_data}
        ...
        <span
          class="condition-badge"
          style="position:absolute;top:10px;right:10px;background:{conditionColors[
            selectedListing.condition
          ]}18;color:{conditionColors[
            selectedListing.condition
          ]};border-color:{conditionColors[
            selectedListing.condition
          ]}44;font-size:0.8rem">{selectedListing.condition}</span
        >
      </div>

      <div
        style="display:flex;gap:16px;margin-bottom:14px;align-items:flex-start"
      >
        <div style="flex:1">
          <p
            class="text-sm"
            style="line-height:1.65;margin-bottom:10px;color:var(--text2)"
          >
            {selectedListing.desc}
          </p>
          <div class="listing-tags">
            {#each selectedListing.tags as t}<span
                class="tag {t === 'Eco-friendly' ? 'green' : ''}">{t}</span
              >{/each}
          </div>
        </div>
        <div style="text-align:right;flex-shrink:0">
          <div class="text-sm text-muted">Listed by</div>
          <div style="font-weight:700;margin-top:2px">
            {selectedListing.user}
          </div>
          <div class="stars" style="font-size:0.8rem">★★★★★</div>
        </div>
      </div>

      <div
        style="background:var(--surface2);border:1.5px solid var(--border);border-radius:var(--radius-sm);padding:12px 14px;margin-bottom:12px"
      >
        <div class="text-muted text-sm" style="margin-bottom:3px">
          Looking for:
        </div>
        <div style="font-weight:700;color:var(--text)">
          {selectedListing.wants}
        </div>
      </div>
      ...
      {#if !selectedListing.mine}
        <div style="margin-bottom:16px">
          <button
            class="btn btn-outline btn-sm"
            onclick={() => getAISuggestion(selectedListing.id)}
            disabled={aiLoading}
            style="width:100%"
          >
      ...
      <div class="section-header" style="margin-bottom:8px">
        <div class="section-title">Propose a Trade</div>
      </div>
      <div style="display:flex;flex-wrap:wrap;gap:8px;margin-bottom:16px">
        {#each myItems as i}
          <button
            class="proposal-item"
            style="cursor:pointer;{selectedOfferItem === i.id
              ? 'border-color:var(--accent);background:var(--accent-light);color:var(--accent2);'
              : ''}"
            onclick={() => selectOffer(i.id)}
          >
            <span>{i.emoji}</span>{i.title}
          </button>
        {/each}
        {#if myItems.length === 0}<span class="text-muted text-sm"
            >No listings yet — <button
              onclick={() => {
                modals.listingDetail = false;
                modals.newListing = true;
              }}
              style="color:var(--accent);font-weight:600;background:none;border:none;cursor:pointer;text-decoration:underline"
      ...
      <button
        class="btn btn-primary w-full"
        style={selectedListing.mine ? "opacity:0.4" : ""}
        disabled={selectedListing.mine}
        onclick={proposeTrade}
      >
        {selectedListing.mine
          ? "This is your listing"
          : "Send Trade Proposal & Open Chat"}
      </button>
    </div>
  {/if}
</div>
...
<!-- svelte-ignore a11y_no_static_element_interactions -->
<div
  class="modal-overlay {showReviewModal ? 'open' : ''}"
  onclick={(e) => {
    if ((e.target as HTMLElement).classList.contains("modal-overlay"))
      showReviewModal = false;
  }}
>
  <div class="modal" style="max-width:440px">
    <div class="modal-header">
      ...
      <button class="close-btn" onclick={() => (showReviewModal = false)}
        >×</button
      >
    </div>
    <p style="font-size:0.88rem;color:var(--text3);margin-bottom:18px">
      How was your trade with <strong style="color:var(--text)"
        >{reviewTarget.revieweeName}</strong
      >?
    </p>
    <div class="form-group">
      <label>Rating</label>
      <div style="display:flex;gap:8px;margin-top:6px">
        {#each [1, 2, 3, 4, 5] as s}
          <button
            onclick={() => (reviewForm.score = s)}
            style="font-size:1.6rem;background:none;border:none;cursor:pointer;opacity:{reviewForm.score >=
            s
              ? 1
              : 0.25};transition:opacity 0.15s">★</button
          >
        {/each}
      </div>
    </div>
    <div class="form-group">
      <label for="review-comment">Comment (optional)</label><textarea
        id="review-comment"
        bind:value={reviewForm.comment}
        placeholder="Describe how the trade went…"
        rows="3"
      ></textarea>
    </div>
    <div style="display:flex;gap:10px">
      ...
      >
      <button class="btn btn-outline" onclick={() => (showReviewModal = false)}
        >Skip</button
      >
    </div>
  </div>
...
<!-- svelte-ignore a11y_no_static_element_interactions -->
<div
  class="modal-overlay {showDeleteModal ? 'open' : ''}"
  onclick={(e) => {
    if ((e.target as HTMLElement).classList.contains("modal-overlay"))
...
>
  <div class="modal" style="max-width:460px">
    <div class="modal-header">
      ...
      <button class="close-btn" onclick={() => (showDeleteModal = false)}
        >×</button
      >
    </div>
    <div
      style="background:rgba(225,29,72,0.06);border:1.5px solid rgba(225,29,72,0.2);border-radius:var(--radius-sm);padding:14px 16px;margin-bottom:20px"
    >
      <div
        style="font-weight:700;font-size:0.88rem;color:var(--warn);margin-bottom:6px"
      >
        ⚠ This action is permanent and cannot be undone
      </div>
      <ul
        style="font-size:0.82rem;color:#dc2626;line-height:1.7;padding-left:18px;margin:0"
      >
    ...
    <div style="display:flex;gap:10px">
      <button
        ...
        >{deleteLoading ? "Deleting…" : "Permanently Delete My Account"}</button
      >
