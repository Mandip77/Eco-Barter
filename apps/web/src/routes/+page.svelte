<script lang="ts">
    ...
    if (authState.user) {
        try {
            ...
            if (repResp.ok) {
                ...
                userReputation = repData.eigentrust_score;
                userRank = repData.rank;
            }
        } catch(e){}
    }
        ...
        try {
            ...
            if (propResp.ok) {
                ...
                chats = proposals.map((p: any) => ({
                    id: p.ID,
                    with: `Loop Nodes: ${p.user_a}, ${p.user_b}, ${p.user_c}`,
                    initials: 'TR',
                    item: `Match Loop #${p.ID}`,
                    online: true,
                    messages: [],
                    proposal: { me: [p.item_a], them: [p.item_b, p.item_c] }
                }));
  ...
  let selectedListingId: number | null = $state(null);
  ...
  function proposeTrade() {
    ...
    const them = selectedListing;
    
    showToast(`Trade preference registered for ${them.title}! The EcoBarter Matrix is calculating loops...`);
  ...
  function acceptTrade(id: number) {
    ...
    chats[chatIndex].messages = [...chats[chatIndex].messages, { from: 'me', text: '✅ Trade accepted! Let\'s coordinate the swap pickup.', time: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) }];
    chats = [...chats];
    showToast('🎉 Trade accepted! Eco Score updated.');
  }

  async function getAISuggestion(id: number) {
    showToast('AI suggestions simulated locally for this prototype.');
  }
  ...
  async function addListing() {
    ...
    try {
      ...
      if (resp.ok) {
        ...
        showToast(`"${newListing.title}" listed!`);
...
<nav>
  ...
  <div class="nav-links">
    ...
    <button class="nav-btn {currentPage === 'chat' ? 'active' : ''}" onclick={() => setPage('chat')}>Negotiation <span class="badge">2</span></button>
...
<div class="page {currentPage === 'browse' ? 'active' : ''}" id="page-browse">
  ...
  <div id="listings-section">
    <div class="section-header">
      ...
      <span class="text-sm text-muted" id="listing-count">{filteredListings.length} listings</span>
    </div>
    <div class="filters">
      <input type="text" class="search-input" placeholder="Search listings…" oninput={handleSearch}>
      <div id="filter-chips" style="display:flex;gap:7px;flex-wrap:wrap">
    ...
    <div class="card-grid">
      {#each filteredListings as l}
        <button class="listing-card text-left" onclick={() => openListing(l.id)}>
...
<div class="page {currentPage === 'chat' ? 'active' : ''}" id="page-chat">
  <div class="chat-layout">
    <div class="chat-sidebar">
      ...
      {#each chats as c}
        <button class="chat-item text-left w-full {c.id === activeChatId ? 'active' : ''}" onclick={() => { activeChatId = c.id; scrollToBottom(); }}>
    ...
    <div class="chat-main" id="chat-main">
      {#if activeChatId && activeChat}
        <div class="chat-header">
          ...
          <div><div style="font-weight:700;font-size:0.95rem">{activeChat.with}</div><div class="text-sm text-muted">{activeChat.item}</div></div>
        ...
        <div class="chat-messages">
          {#if activeChat.proposal}
            <div class="proposal-bar">
              <span class="text-sm" style="color:var(--accent);font-weight:600;flex-shrink:0">📦 Proposal</span>
...
<div class="page {currentPage === 'profile' ? 'active' : ''}" id="page-profile">
  <div class="profile-header">
    ...
    <div class="profile-info">
      ...
      <div class="stars" style="color: {userReputation > 50 ? '#10b981' : '#fbbf24'}">★★★★★</div>
      <div class="stat-row">
        <div class="stat"><div class="stat-val">{myItems.length}</div><div class="stat-label">Listings</div></div>
        <div class="stat"><div class="stat-val">{userReputation}</div><div class="stat-label">Trust Score</div></div>
        <div class="stat"><div class="stat-val">{chats.length}</div><div class="stat-label">Active Nodes</div></div>
      </div>
      <div class="mt-12"><span class="eco-score">🌱 84 kg CO₂ saved</span></div>
    </div>
  </div>
  <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px">
    <div>
      ...
      {#each myItems as l}
        <div class="card" style="margin-bottom:9px;display:flex;align-items:center;gap:12px">
    ...
    <div>
      ...
      <div class="review-card"><div class="stars" style="font-size:0.82rem">★★★★★</div><p class="text-sm mt-12">"Super smooth trade! Alex was communicative and the vintage lamp was exactly as described."</p><p class="text-sm text-muted mt-12">— Morgan T. · 3 days ago</p></div>
      <div class="review-card"><div class="stars" style="font-size:0.82rem">★★★★★</div><p class="text-sm mt-12">"Great swap, would trade again. Showed up on time with the board games as promised."</p><p class="text-sm text-muted mt-12">— Priya K. · 1 week ago</p></div>
      <div class="review-card"><div class="stars" style="font-size:0.82rem">★★★★☆</div><p class="text-sm mt-12">"Friendly and honest. The guitar had a small scratch not mentioned, but we worked it out."</p><p class="text-sm text-muted mt-12">— Sam R. · 2 weeks ago</p></div>
    </div>
  </div>
</div>
...
<div class="modal-overlay {modals.newListing ? 'open' : ''}" onclick={closeModals}>
  <div class="modal">
    ...
    <div class="form-group"><label>Item / Service Name</label><input type="text" bind:value={newListing.title} placeholder="e.g. Vintage Polaroid Camera"></div>
    <div class="form-group"><label>Category</label><select bind:value={newListing.cat}>
      {#each categories.filter(c => c !== 'All') as c}<option>{c}</option>{/each}
    </select></div>
    <div class="form-group"><label>Description</label><textarea bind:value={newListing.desc} placeholder="Describe condition, details, etc."></textarea></div>
    <div class="form-group"><label>Looking to trade for…</label><input type="text" bind:value={newListing.wants} placeholder="e.g. Camera lenses, or open to offers"></div>
    <div class="form-group"><label>Emoji Icon</label><input type="text" bind:value={newListing.emoji} placeholder="📷" maxlength="4" style="width:76px"></div>
...
<div class="modal-overlay {modals.listingDetail && selectedListing ? 'open' : ''}" onclick={closeModals}>
  {#if selectedListing}
    <div class="modal" style="max-width:600px">
      ...
      <div style="font-size:4.5rem;text-align:center;padding:20px;background:var(--surface2);border-radius:var(--radius-sm);margin-bottom:16px">{selectedListing.emoji}</div>
      <div class="flex gap-12" style="margin-bottom:14px;align-items:flex-start">
        ...
        <div style="text-align:right;flex-shrink:0"><div class="text-muted text-sm">Listed by</div><div style="font-weight:600;margin-top:2px">{selectedListing.user}</div><div class="stars" style="font-size:0.8rem">★★★★★</div></div>
      </div>
      <div style="background:var(--surface2);border:1px solid var(--border);border-radius:var(--radius-sm);padding:12px 14px">
        ...
        <div style="font-weight:600;color:var(--text)">{selectedListing.wants}</div>
      </div>
      <hr class="divider">
      <div class="section-header" style="margin-bottom:8px"><div class="section-title">Propose a Trade</div></div>
      <p class="text-sm text-muted" style="margin-bottom:11px">Select one of your listings to offer in exchange:</p>
      <div style="display:flex;flex-wrap:wrap;gap:8px;margin-bottom:15px">
        {#each myItems as i}
          <button class="proposal-item" style="cursor:pointer; transition:all 0.15s; {selectedOfferItem === i.id ? 'border-color:var(--accent-mid);background:var(--accent-light);' : ''}" onclick={() => selectOffer(i.id)}>
            <span>{i.emoji}</span>{i.title}
          </button>
        {/each}
        {#if myItems.length === 0}
          <span class="text-muted text-sm">No listings yet.</span>
        {/if}
      </div>
