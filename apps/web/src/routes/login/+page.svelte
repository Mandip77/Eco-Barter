...
<div class="min-h-screen flex items-center justify-center p-6 relative overflow-hidden">
  ...
  <div class="w-full max-w-lg relative z-10">
    <div class="text-center mb-12">
      ...
      <p class="text-[var(--text2)] mt-4 text-lg font-medium opacity-80">The future of sustainable trading</p>
    </div>

    <div class="bg-white/80 backdrop-blur-xl rounded-[32px] p-10 md:p-12 border border-white shadow-[var(--shadow-premium)]">
      <h2 class="text-3xl font-extrabold mb-10 text-[var(--text)] text-center tracking-tight">
        {isLogin ? 'Welcome back' : 'Join the movement'}
      </h2>
      
      {#if errorMsg}
        <div class="bg-red-500/10 border border-red-500/20 text-red-600 px-5 py-4 rounded-xl mb-8 text-sm flex items-center gap-3 animate-in fade-in slide-in-from-top-2">
      ...
      <form onsubmit={handleSubmit} class="flex flex-col gap-6">
        {#if !isLogin}
          <div class="flex flex-col gap-2.5">
            <label class="text-xs font-bold uppercase tracking-widest text-[var(--text3)] ml-1" for="username">Username</label>
            <input 
              bind:value={username}
              type="text" 
              id="username" 
        ...
        <div class="flex flex-col gap-2.5">
          <label class="text-xs font-bold uppercase tracking-widest text-[var(--text3)] ml-1" for="email">Email Address</label>
          <input 
            bind:value={email}
            type="email" 
            id="email" 
        ...
        <div class="flex flex-col gap-2.5">
          <div class="flex items-center justify-between">
            <label class="text-xs font-bold uppercase tracking-widest text-[var(--text3)] ml-1" for="password">Password</label>
          </div>
          <input 
            bind:value={password}
            type="password" 
